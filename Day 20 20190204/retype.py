...
def change_personal_info(account_dic, username):
    ...
    1. 把这个人的每个信息打印出来，让其选择改哪个字段，用户选择了数字，正好是字段的索引，这样直接把字段找出来改掉就可以了
    2. 改完后，还要把这个新数据重新写回到account.txt，由于改完后的新数据是dict类型，还需把dict转成字符串后，再写会硬盘
    :param account_dic:
    :param username:
    :return: None
    """
    ...
    choice = input("[select colunm id to change]:").strip()
    if choice.isdigit():
        choice = int(choice)
